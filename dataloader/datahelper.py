...
class BucketIterator(object):
    def __init__(self, data, opt=None, batch_size=32, shuffle=True, test=False, position=False):
    ...
    def __iter__(self):
        if self.shuffle:
            self.data = self.data.sample(frac=1).reset_index(dorp=True)
...
def log_time_delta(func):
    ...
    def _deco(*args, **kwargs):
        ...
        delta = end - start
        print("{} runed {%.2f} seconds".format(func.__name__, delta))
        return ret
    return _deco
    ...
    with open(filename, encoding='utf-8') as f:
        for line in tqdm(f):
            ...
            if len(items) == 2:
                ...
                print("For pretrained embedding, embedding_size : {} ; vocab_Size : {}".format(embedding_size, vocab_size))
            else:
                word = items[0]
                for word in alphabet:
                    vectors[word] = items[1:]
    print("vocab size in alphbet : {}".format(len(alphabet)))
    print("vocab size in embedding:{}".format(len((vectors))))
...
def vectors_lookup(vectors, vocab, dim):
    ...
    print("{} words in embedding".format(count))
    return embedding
...
def loadData(opt, embedding=True):
    ...
    # decide the padding for the sentence
    # if do not design the max_sent_len
    ...
    for data in datas:
        data["text"] = data["text"].apply(
            lambda text: [alphabet.get(word, alphabet.unknow_token) for word in text[:opt.max_seq_len]] + [
                                                                                                              alphabet.padding_token] * int(
                opt.max_seq_len - len(text)))
